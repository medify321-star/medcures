#!/usr/bin/env python3
"""
Script to create a new pharmacopoeia database with 240+ drugs
"""
...
def parse_drugs_from_text(text):
    ...
    for block in blocks:
        ...
        for line in lines:
            line = line.strip()
            if line and not line.startswith("Drug common name:"):
                if ":" in line:
                    ...
                    if key == "Drug common name":
                        ...
                        drug_data["Side Effects"] = [e.strip() for e in value.split(",")]
                    elif key == "Citation":
                        drug_data["Citation"] = value
                    elif key == "Disclaimer":
                        drug_data["Disclaimer"] = value
        
        # Add the first line (drug name) to the data
        first_line = block.split("\n")[0].strip()
        if first_line and "Drug" not in drug_data:
            drug_data["Drug"] = first_line
        
        # Only add if we have the minimum required fields
        if "Drug" in drug_data and drug_data["Drug"]:
...
print(f"✓ Total drugs added: {len(parsed_drugs)}")
print(f"✓ Sample drugs: {', '.join([d['Drug'] for d in parsed_drugs[:3]])}")
